...
class AddCreditDialog : DialogFragment() {
    interface Listener { fun onTransaction(type: TransactionType, amount: Double, note: String?, date: Long) }
    private var selectedDate = System.currentTimeMillis()
    override fun onCreateDialog(state: android.os.Bundle?) = androidx.appcompat.app.AlertDialog.Builder(requireContext()).setTitle("Credit Given").setView(createContent()).setPositiveButton("Add Credit", null).setNegativeButton("Cancel", null).create()
    private fun createContent(): View {
        ...
        refreshDate()
        dateButton.setOnClickListener { pickDate { selectedDate = it; refreshDate() } }
        return view
    }
    override fun onStart() {
        ...
        d.getButton(android.app.AlertDialog.BUTTON_POSITIVE).setOnClickListener {
            ...
            (activity as? Listener)?.onTransaction(TransactionType.CREDIT, amount, d.findViewById<TextInputEditText>(R.id.noteInput)?.text.toString().ifBlank { null }, selectedDate)
            dismiss()
        }
    }
    private fun pickDate(done: (Long) -> Unit) {
        val c = Calendar.getInstance().apply { timeInMillis = selectedDate }
        DatePickerDialog(requireContext(), { _, y, m, d -> done(Calendar.getInstance().apply { set(y, m, d) }.timeInMillis) }, c.get(Calendar.YEAR), c.get(Calendar.MONTH), c.get(Calendar.DAY_OF_MONTH)).show()
    }
}
